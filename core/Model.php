<?php
    ...
    abstract class Model {
        ...
        final public function getAllVisible(): array {
            $tableName = $this->getTableName();
            $sql = 'SELECT * FROM ' . $tableName . ' WHERE is_visible = 1;';
            $prep = $this->dbc->getConnection()->prepare($sql);
            $res = $prep->execute();
            $items = [];
            if($res) {
        ...
        final public function deleteById(int $id) {
            ...
            $sql = "UPDATE {$tableName} SET {$tableName}.is_visible = 0 WHERE {$tableName}.category_id = ?;";
            $prep = $this->dbc->getConnection()->prepare($sql);
            return $prep->execute([$id]);

        }

        final public function deleteItemById(int $id) {
            $tableName = $this->getTableName();
            
            $sql = "UPDATE {$tableName} SET {$tableName}.is_visible = 0 WHERE {$tableName}.item_id = ?;";
            $prep = $this->dbc->getConnection()->prepare($sql);
            return $prep->execute([$id]);
